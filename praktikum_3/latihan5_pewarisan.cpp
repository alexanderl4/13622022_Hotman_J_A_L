/*
 * EXPLORATION ANSWER:
 * Changing 'protected' to 'private' in the Base Class causes the compiler to immediately reject the program.
 * The Derived Class (SensorSuhu) throws an error because it is not allowed to steal or view 'private' data,
 * even if that data belongs to its own Parent class.
 */
#include <iostream>
using namespace std;
...
public:  
    ...
    void kalibrasiDanCetak() {  
        ...
        cout << "Sensor Suhu ID " << id_sensor << " | Kalibrasi: " << kalibrasi << " C\n";  
    }  
};
